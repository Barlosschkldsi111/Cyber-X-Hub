	...
	function k:AddTab(_)
		...
		function h:AddSlider(_)
			...
			local sliderFrame = p:Create("Frame",{
				...
				p:Create("UIListLayout",{Name="UIListLayout",VerticalAlignment=Enum.VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,5)}),
				p:Create("UIPadding",{Name="UIPadding",PaddingTop=UDim.new(0,13),PaddingBottom=UDim.new(0,13)})
			})

			local sliderTitle = p:Create("TextLabel",{
				Name="SliderTitle",
				FontFace=p.Settings.FontFace or Font.new("rbxasset://fonts/families/GothamSSm.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal),
				Text=h.Title,
				TextColor3=Color3.fromRGB(240,240,240),
				TextSize=13,
				TextXAlignment=Enum.TextXAlignment.Left,
			...
			function f:SetValue(v,bypass)
				...
				o:Animation(sliderValue,{Size=UDim2.fromScale((v-h.Min)/(h.Max-h.Min),1)},p.AnimationSpeed,p.EasingStyle.Quad,p.EasingDirection.Out)
				if not bypass then
					h.Default=v
				end
				...
				if f.Callback then f.Callback(v) end
			end

			function f:SetMin(v)
				h.Min=v
				if h.Default<h.Min then
					self:SetValue(h.Min)
				else
					self:SetValue(h.Default,true)
				end
			end

			function f:SetMax(v)
				h.Max=v
				if h.Default>h.Max then
					self:SetValue(h.Max)
				else
					self:SetValue(h.Default,true)
				end
			...
			function f:SetDefault(v)
				h.Default=math.clamp(v,h.Min,h.Max)
				self:SetValue(h.Default)
			end

			function f:OnChanged(cb)
				f.Callback=cb
				if f.Callback then f.Callback(h.Default) end
				return f
			end

			if h.Default then f:SetValue(h.Default) end
